...
Waybar (bottom bar with workspaces per monitor, app launchers, hover volume slider, headset battery, network, bluetooth, a CPU/GPU metrics gauge, brightness and battery), the fuzzel launcher on Super+D in the DOOM login palette, mako notifications, and waypaper with the awww daemon for the wallpaper. Shared by both compositors; the entry points are `config.jsonc` (Sway) and `config-hyprland.jsonc` (Hyprland), each one bar per output including the shared `modules-*.jsonc`. The brightness module on a monitor's bar shows and scrolls that monitor's own brightness — the laptop panel through `/sys/class/backlight`, external monitors over DDC/CI (`ddcutil`; `--enable-bar` re-triggers its udev rule so the `/dev/i2c-*` buses are reachable from the session). Next to the volume sits the battery of the connected Bluetooth headset, hidden when no device reports one — a headset reports it over HFP, and BlueZ only turns that into `org.bluez.Battery1` with `Experimental = true` in `/etc/bluetooth/main.conf`, which `--enable-bar` sets for you. Includes the DOOM fonts and wallpaper.
